...
use DIISH\SComDisBundle\Form\OutCARPHAReportType;
...
class ReportController extends AppController
{
    ...
    {              
        ...
        $filename = $this->get('kernel')->locateResource('@DIISHSComDisBundle/Data/SComdis_DailyTally.xls');
    ...
    public function outCARPHAReportAction(Request $request)
    {
        ...
        $form = $this->createForm(new OutCARPHAReportType(), null);
        ...
        if ($request->getMethod() === 'POST') {
            ...
            if ($form->isValid()) {
                ...
                $filename = $this->get('kernel')->locateResource('@DIISHSComDisBundle/Data/SComdis_CARPHAWEEKLY.xls');
                if (!file_exists($filename)) {
                    throw $this->createNotFoundException('No template found');
                }
